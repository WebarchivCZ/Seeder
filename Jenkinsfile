      ...
      stage('Deploy to production') {
        ...
        steps {
          ...
          sh '''#!/usr/bin/env bash
            ...
            ssh -o "StrictHostKeyChecking=no" -i ${SSH_CREDS} ${SSH_CREDS_USR}@10.3.0.50 sudo docker-compose -f docker-compose-prod.yml -p seeder pull
            ssh -o "StrictHostKeyChecking=no" -i ${SSH_CREDS} ${SSH_CREDS_USR}@10.3.0.50 sudo docker-compose -f docker-compose-prod.yml -p seeder up -d --remove-orphans
          '''
        }
      }
    }
